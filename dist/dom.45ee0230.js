...
window.dom = {
    ...
    append: function append(parent, node2) {
        parent.appendChild(node2); //新增一个儿子node2
    },
    wrap: function wrap(node2, parent) {
        dom.before(node2, parent);
        dom.append(parent, node2);
    },
    //在node2上新增一个爸爸parent
    remove: function remove(node2) {
        node2.parentNode.removeChild(node2);
        return node2; //删除node2节点
    },
    empty: function empty(node) {
        var childNodes = node.childNodes; //const childNodes = node.ChildNodes 简写成 const{childNodes} =node
...
//# sourceMappingURL=/dom.45ee0230.map